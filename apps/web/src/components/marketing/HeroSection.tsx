...
/* ── Aurora background (CSS-only, no lib needed) ─────────────────── */
function AuroraBackground() {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden>
      ...
      {/* Aurora blobs */}
      <div
        className="absolute w-[900px] h-[900px] rounded-full opacity-25 blur-[120px]"
        style={{
      ...
      />
      <div
        className="absolute w-[600px] h-[600px] rounded-full opacity-10 blur-[90px]"
        style={{
      ...
      <style>{`
        ...
        @keyframes aurora4 {
          0%,100% { transform: translate(0,0) scale(1); }
          35%      { transform: translate(-60px,-80px) scale(1.06); }
          65%      { transform: translate(80px,50px) scale(0.94); }
        }
      `}</style>
    </div>
  )
...
function DashboardMockup() {
  return (
    <div className="relative w-full max-w-xl mx-auto lg:mx-0">
      {/* Glow */}
      <div className="absolute -inset-6 bg-green-500/10 rounded-3xl blur-2xl" />

      {/* Browser shell */}
      <div
        className="relative rounded-2xl overflow-hidden border"
        style={{ borderColor: 'rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.04)', backdropFilter: 'blur(12px)' }}
      >
        {/* Chrome */}
        <div className="flex items-center gap-2 px-4 py-3 border-b" style={{ borderColor: 'rgba(255,255,255,0.08)', background: 'rgba(0,0,0,0.3)' }}>
          <div className="flex gap-1.5">
            <div className="w-3 h-3 rounded-full bg-red-500/80" />
            <div className="w-3 h-3 rounded-full bg-yellow-500/80" />
            <div className="w-3 h-3 rounded-full bg-green-500/80" />
          </div>
          <div className="flex-1 rounded-lg px-3 py-1 text-xs ml-2" style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.4)' }}>
            app.vetcare.com.br/dashboard
          </div>
        </div>

        {/* App content */}
        <div className="p-4" style={{ background: 'rgba(2,10,5,0.8)' }}>
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            {[
              { icon: Calendar, label: 'Consultas', value: '12', color: 'text-blue-400', bg: 'rgba(59,130,246,0.1)' },
              { icon: Users, label: 'Pacientes', value: '248', color: 'text-green-400', bg: 'rgba(34,197,94,0.1)' },
              { icon: DollarSign, label: 'Receita', value: 'R$8.4k', color: 'text-purple-400', bg: 'rgba(168,85,247,0.1)' },
            ].map((s) => (
          ...
          <div className="rounded-xl p-3 mb-3" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <div className="flex items-end gap-1 h-12">
              {[35, 60, 45, 75, 50, 90, 68].map((h, i) => (
                <div key={i} className={`flex-1 rounded-t-sm transition-all ${i === 5 ? 'bg-green-500' : 'bg-green-500/25'}`} style={{ height: `${h}%` }} />
              ))}
            </div>
          </div>
          ...
          <div className="rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
            <div className="text-xs font-semibold text-white/70 mb-2 flex items-center gap-1.5">
              <Calendar className="w-3 h-3 text-green-400" /> Próximas consultas
            </div>
            {[
              { pet: 'Thor', owner: 'Ana Lima', time: '15:00' },
              { pet: 'Mimi', owner: 'João Silva', time: '15:30' },
            ].map((a) => (
              <div key={a.pet} className="flex items-center gap-2 py-1.5" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <div className="w-6 h-6 rounded-full bg-green-500/20 flex items-center justify-center text-xs font-bold text-green-400">{a.pet[0]}</div>
                <div className="flex-1 text-xs text-white/70">{a.pet} · {a.owner}</div>
                <div className="text-xs font-semibold text-white/60">{a.time}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Floating badges */}
      <div className="absolute -bottom-5 -left-5 px-4 py-3 rounded-2xl flex items-center gap-3 shadow-2xl" style={{ background: 'rgba(255,255,255,0.08)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.12)' }}>
      ...
      <div className="absolute -top-5 -right-5 px-4 py-3 rounded-2xl flex items-center gap-3 shadow-2xl" style={{ background: 'rgba(255,255,255,0.08)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.12)' }}>
    ...
    <section className="relative min-h-screen flex items-center overflow-hidden pt-16">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-32 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          {/* Left */}
          <div>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-8 text-xs font-semibold" style={{ background: 'rgba(22,163,74,0.15)', border: '1px solid rgba(22,163,74,0.3)', color: '#4ade80' }}>
            ...
            <h1 className="text-5xl sm:text-6xl lg:text-7xl font-black text-white leading-[1.05] tracking-tight mb-6">
              ...
              <span className="relative">
                <span className="text-transparent bg-clip-text" style={{ backgroundImage: 'linear-gradient(135deg, #4ade80 0%, #22c55e 50%, #0d9488 100%)' }}>
                  veterinária
                </span>
              </span>
            ...
            <ul className="space-y-3 mb-10">
              {['14 dias grátis, sem cartão de crédito', 'PIX, boleto e cartão — 100% brasileiro', 'Dados protegidos pelo LGPD no Brasil'].map((item) => (
            ...
            <div className="flex items-center gap-4">
              <div className="flex -space-x-2">
                {['A','M','C','P','L'].map((l, i) => (
                  <div key={i} className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white border-2" style={{ background: `hsl(${140 + i*15},60%,40%)`, borderColor: '#020a05' }}>{l}</div>
                ))}
              </div>
              <div>
